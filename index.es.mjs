...
const selectorProcessor = selectorParser(selectors => {
  ...
  let nonHoverSelectors = selectors.map(selector => `${selector}`).filter(selectorExpression => !hoverSelectors.includes(selectorExpression));
  return {
    hoverSelectors,
    nonHoverSelectors
  };
...
module.exports = ({
  ...
  return {
    ...
    Rule(rule, {
      ...
      if (!rule.selector.includes(':hover') || isAlreadyNested(rule) || rule.selector.includes(fallbackSelector)) {
        return;
      }
      ...
      let mediaQuery = createMediaQuery(rule.clone({
